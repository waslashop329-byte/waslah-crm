...
import { deleteCouponAction, type PromotionActionState } from "@/app/(dashboard)/promotions/actions";
...
export function DeleteCouponButton({ couponId, couponCode }: { couponId: string; couponCode: string }) {
  ...
  const [state, formAction, isPending] = useActionState(deleteCouponAction, initialState);
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t("deleteConfirmTitle", { name: couponCode })}</AlertDialogTitle>
